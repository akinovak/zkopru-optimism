...
library Types {
    ...
    function hash(Transaction memory transaction)
        internal
        pure
        returns (bytes32)
    {
    ...
    function toBytes(Inflow memory inflow)
        internal
        pure
    ...
    function toBytes(Outflow memory outflow)
        internal
        pure
        returns (bytes memory)
    {
    ...
    function toBytes(AtomicSwap memory swap)
        internal
        pure
        returns (bytes memory)
    {
        return abi.encodePacked(swap.binder, swap.counterpart);
    }
    ...
    function withdrawalNote(Outflow memory outflow)
        internal
        pure
        returns (bytes32)
    {
    ...
    function hash(MassDeposit memory massDeposit)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(massDeposit.merged, massDeposit.fee));
    }

    function hash(MassMigration memory massMigration)
        internal
        pure
        returns (bytes32)
    {
    ...
    function root(MassDeposit[] memory massDeposits)
        internal
        pure
        returns (bytes32)
    {
    ...
    function root(MassMigration[] memory massMigrations)
        internal
        pure
        returns (bytes32)
    {
    ...
    function root(bytes32[] memory leaves) internal pure returns (bytes32) {
        ...
        for (uint256 i = 0; i < nodes.length; i++) {
            if (hasEmptyLeaf && i == nodes.length - 1) {
                ...
                nodes[i] = keccak256(
                    abi.encodePacked(leaves[i * 2], leaves[i * 2 + 1])
                );
            }
        }
        return root(nodes);
    ...
    function isEmpty(PublicData memory publicData)
        internal
        pure
        returns (bool)
    {
