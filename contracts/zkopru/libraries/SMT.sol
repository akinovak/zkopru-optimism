...
library SMT254 {
    ...
    bytes32 public constant EXIST = bytes32(uint256(1));
    // in Solidity: keccak256(abi.encodePacked(bytes32(0)))
    // in Web3JS: soliditySha3(0)
    bytes32 public constant NON_EXIST = 0;
    ...
    function merkleProof(
        ...
        require(
            calculateRoot(leaf, value, siblings) == root,
    ...
    function fill(
        ...
        require(
            nonInclusionProof(prevRoot, leaf, siblings),
            "Failed to build the previous root using the leaf and its sibling"
        );
